...
module.exports.run = function ({ api, event, args, getText }) {
  ...
  if (!content) {
    ...
    if (
      Number.isNaN(contentHour) ||
      Number.isNaN(contentMinute) ||
      contentHour > 23 ||
      contentMinute > 59 ||
      contentHour < 0 ||
      contentMinute < 0 ||
    ...
    var sleepTime = getTime
      .replace(`${hour}:`, `${contentHour}:`)
      .replace(`${minute}:`, `${contentMinute}:`)
    for (var i = 1; i < 7; i++)
      wakeTime.push(
        moment(sleepTime)
          .tz('Asia/Ho_Chi_Minh')
    ...
    return api.sendMessage(
      getText('returnTimeSet', content, wakeTime.join(', ')),
      threadID,
      messageID
    )
  }
