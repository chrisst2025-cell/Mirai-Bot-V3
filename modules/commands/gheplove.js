...
module.exports.run = async ({ api, event, args, Users }) => {
  ...
  const chuc = mung[Math.floor(Math.random() * mung.length)]
  if (!args[0]) {
    ...
    if (data.length === 0)
      return api.sendMessage(
        'Rất tiếc! Không tìm thấy nửa đời của bạn 😥',
        event.threadID,
        event.messageID
      )
    ...
    const _url = api.getCurrentUserID(e)

    const getAvatar = (
      await axios.get(
        `https://graph.facebook.com/${e}/picture?height=1500&width=1500&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662
  `,
