...
module.exports.run = async ({ api, event, Users, Threads, Currencies }) => {
  ...
  if (money1 < 1 || Number.isNaN(money1))
    return api.sendMessage(`Bạn không có tiền lỡ bị bắt r lấy gì trả!`, threadID, messageID)
  ...
  if (money2 < 1 || Number.isNaN(money2))
    return api.sendMessage(`Mục tiêu ${name} không có đồng xu dính túi!`, threadID, messageID)
  ...
  if (tile < cc) {
    ...
    return api.sendMessage(
      `Bạn đã cướp ${name} thất bại và mất ${sotienmat}$ :>`,
      threadID,
      messageID
    )
  }
