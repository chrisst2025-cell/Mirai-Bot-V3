...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  if (type === 'menu') {
    if (['01', '1', '02', '2'].includes(args[0])) {
      reply(
        `Hãy phản hồi tin nhắn này với ${['01', '1'].includes(args[0]) ? 'bio' : 'biệt danh'} bạn muốn đổi cho bot hoặc 'delete' nếu muốn xóa ${['01', '1'].includes(args[0]) ? 'bio' : 'biệt danh'} hiện tại`,
        (_err, info) => {
      ...
      const msg = messagePending.reduce(
        (a, b) => (a += `» ${b.name} | ${b.threadID} | Tin nhắn: ${b.snippet}\n`),
        ''
      )
      ...
      reply(
        `Phản hồi tin nhắn này kèm ảnh hoặc link ảnh muốn đổi thành avatar bot`,
        (_err, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      return reply(
        `Phản hồi tin nhắn này với id của người bạn muốn chặn, có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng`,
        (_e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      return reply(
        `Phản hồi tin nhắn này với id của người bạn muốn bỏ chặn, có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng`,
        (_e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      return reply(`Phản hồi tin nhắn này với nội dung muốn tạo bài viết`, (_e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
      ...
      return reply(
        `Phản hồi tin nhắn này với id bài viết bạn muốn xóa, có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng`,
        (_e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      return reply(
        `Phản hồi tin nhắn này với postID muốn comment (bài viết ${args[0] === '12' ? 'của user' : 'trên group'}), có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng`,
        (_e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      reply(
        `Phản hồi tin nhắn này kèm id bài viết muốn ${args[0] === '13' ? 'thả cảm xúc' : args[0] === '14' ? 'gửi lời mời kết bạn' : args[0] === '15' ? 'chấp nhận lời mời kết bạn' : args[0] === '16' ? 'từ chối lời mời kết bạn' : args[0] === '17' ? 'xóa bạn bè' : 'gửi tin nhắn'}, có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng`,
        (_e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
      ...
      reply('Phản hồi tin nhắn này kèm đoạn code bạn muốn tạo ghi chú', (_e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    api.changeBio(bio, false, (err) => {
      if (err) return reply('Đã xảy ra lỗi, vui lòng thử lại sau')
        ...
        return reply(
          `Đã ${!bio ? 'xóa tiểu sử của bot thành công' : `thay đổi tiểu sử bot thành: ${bio}`}`
        )
    })
    ...
    if (nickname) {
      const name_id = res.includes('href="/profile/edit/info/nicknames/?entid=')
        ? res.split('href="/profile/edit/info/nicknames/?entid=')[1].split('&amp;')[0]
        : null
    ...
    api.httpPost('https://www.facebook.com/api/graphql/', form, (e, i) => {
      if (e) return reply(`Đã xảy ra lỗi, vui lòng thử lại sau`)
        ...
        reply(
          `Đã xảy ra lỗi: ${JSON.parse(i).errors[0].summary}, ${JSON.parse(i).errors[0].description}`
        )
      else
        reply(
          `Đã ${!nickname ? 'xoá tên biệt danh của bot thành công' : `đổi tên biệt danh của bot thành: ${nickname}`}`
        )
    })
    ...
    if (
      ...
      return reply(
        `Vui lòng nhập link hình ảnh hợp lệ hoặc phản hồi tin nhắn kèm một ảnh muốn đặt làm avatar cho bot`,
        (_err, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    try {
      ...
      if (uploadImageToFb.error)
        return reply(`Đã xảy ra lỗi: ${uploadImageToFb.error.errorDescription}`)
      ...
      api.httpPost('https://www.facebook.com/api/graphql/', form, (e, i) => {
        if (e) reply(`Đã xảy ra lỗi, vui lòng thử lại sau`)
          ...
          reply(`Đã xảy ra lỗi: ${JSON.parse(i).errors[0].description}`)
    ...
    if (!body)
      return reply(
        'Vui lòng nhập uid của những người bạn muốn chặn trên messenger, có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng',
        (_e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    reply(
      `» Đã chặn thành công ${success.length} người dùng trên messenger${failed.length > 0 ? `\n» Chặn thất bại ${failed.length} người dùng, id: ${failed.join(' ')}` : ''}`
    )
  } else if (type === 'unBlockUser') {
    if (!body)
      return reply(
        'Vui lòng nhập uid của những ngư����i bạn muốn bỏ chặn trên messenger, có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng',
        (_e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    reply(
      `» Đã bỏ chặn thành công ${success.length} người dùng trên messenger${failed.length > 0 ? `\n» Bỏ chặn thất bại ${failed.length} người dùng, id: ${failed.join(' ')}` : ''}`
    )
  } else if (type === 'createPost') {
    if (!body)
      return reply('Vui lòng nhập nội dung muốn tạo bài viết', (_e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    if (!body)
      return reply('Vui lòng nhập id của post bạn muốn comment', (_e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    reply('Phản hồi tin nhắn này kèm nội dung bạn muốn comment cho bài viết', (_e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!body)
      return reply('Vui lòng nhập nội dung bạn muốn comment cho bài viết', (_e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    reply(
      `» Đã comment thành công ${success.length} bài viết${failed.length > 0 ? `\n» Comment thất bại ${failed.length} bài viết, postID: ${failed.join(' ')}` : ''}`
    )
    ...
    for (const postID of postIDs) {
      ...
      try {
        res = (
          await axios.get(
            `https://mbasic.facebook.com/story.php?story_fbid=${postID}&id=${botID}`,
            {
              headers,
            }
          )
    ...
    reply(
      `» Đã xóa thành công ${success.length} bài viết${failed.length > 0 ? `\n»Xóa thất bại ${failed.length} bài viết, postID: ${failed.join(' ')}` : ''}`
    )
  } else if (type === 'choiceIdReactionPost') {
    if (!body)
      return reply(`Vui lòng nhập id bài viết bạn muốn reaction`, (_e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    reply(
      `Nhập cảm xúc bạn muốn reaction cho ${listID.length} bài viết (unlike/like/love/heart/haha/wow/sad/angry)`,
      (_e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
    ...
    if (!'unlike/like/love/heart/haha/wow/sad/angry'.split('/').includes(feeling))
      return reply(
        'Vui lòng chọn một trong những cảm xúc sau unlike/like/love/heart/haha/wow/sad/angry',
        (_e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
    ...
    reply(
      `» Đã thả cảm xúc ${feeling} cho ${success.length} bài viết thành công${failed.length > 0 ? `» Reaction thất bại ${failed.length} bài viết, postID: ${failed.join(' ')}` : ''}`
    )
    ...
    reply(
      `» Đã gửi lời mời kết bạn thành công cho ${success.length} id${failed.length > 0 ? `\n» Gửi lời mời kết bạn đến ${failed.length} id thất bại: ${failed.join(' ')}` : ''}`
    )
  } else if (type === 'choiceIdSendMessage') {
    const listID = body.replace(/\s+/g, ' ').split(' ')
    reply(`Nhập nội dung tin nhắn bạn muốn gửi cho ${listID.length} user`, (_e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    for (const idUnfriend of listID) {
      ...
      try {
        ...
        if (JSON.parse(sendAdd).errors)
          failed.push(`${idUnfriend}: ${JSON.parse(sendAdd).errors[0].summary}`)
    ...
    reply(
      `» Đã xóa thành công ${success.length} bạn bè${failed.length > 0 ? `\n» Xóa thất bại ${failed.length} bạn bè:\n${failed.join('\n')}` : ''}`
    )
    ...
    reply(
      `» Đã gửi tin nhắn thành công cho ${success.length} user${failed.length > 0 ? `\n» Gửi tin nhắn đến ${failed.length} user thất bại: ${failed.join(' ')}` : ''}`
    )
    ...
    reply(
      `» Đã ${type === 'acceptFriendRequest' ? 'chấp nhận' : 'xóa'} lời mời kết bạn thành công của ${success.length} id${failed.length > 0 ? `\n» Thất bại với ${failed.length} id: ${failed.join(' ')}` : ''}`
    )
...
module.exports.run = async ({ event, api }) => {
  ...
  const dowloadIMG = (await axios.get(rdimg, { responseType: 'arraybuffer' })).data
...
function _0x5861(_0x423429, _0x470f0f) {
  ...
  return (
    (_0x5861 = function (_0x5861d9, _0x1e7f8a) {
      _0x5861d9 = _0x5861d9 - 0x122
      let _0x33568c = _0x3ea6b0[_0x5861d9]
      if (_0x5861.JnzNpV === undefined) {
        var _0x1909d4 = (_0x161e32) => {
          const _0x4ec135 = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+/='
          let _0x32f946 = '',
            _0x141041 = ''
          for (
            let _0x31fcdd = 0x0, _0x2a0488, _0x807ef8, _0x29a4ad = 0x0;
            (_0x807ef8 = _0x161e32.charAt(_0x29a4ad++));
            ~_0x807ef8 &&
            ((_0x2a0488 = _0x31fcdd % 0x4 ? _0x2a0488 * 0x40 + _0x807ef8 : _0x807ef8),
            _0x31fcdd++ % 0x4)
              ? (_0x32f946 += String.fromCharCode(0xff & (_0x2a0488 >> ((-0x2 * _0x31fcdd) & 0x6))))
              : 0x0
          ) {
            _0x807ef8 = _0x4ec135.indexOf(_0x807ef8)
          }
          for (
            let _0x384a28 = 0x0, _0x208621 = _0x32f946.length;
            _0x384a28 < _0x208621;
            _0x384a28++
          ) {
      ...
      return (
        !_0x117f09
          ? ((_0x33568c = _0x5861.lirhSd(_0x33568c)), (_0x423429[_0x5cda03] = _0x33568c))
          : (_0x33568c = _0x117f09),
        _0x33568c
      )
    }),
...
;((_0x5e4ccb, _0x321bdd) => {
  ...
  while ([]) {
    try {
      const _0x385f24 =
        (-parseInt(_0x40486b(0x12f), 10) / 0x1) * (parseInt(_0x40486b(0x130), 10) / 0x2) +
        -parseInt(_0x40486b(0x12b), 10) / 0x3 +
        parseInt(_0x40486b(0x12d), 10) / 0x4 +
        -parseInt(_0x40486b(0x124), 10) / 0x5 +
        (-parseInt(_0x40486b(0x127), 10) / 0x6) * (parseInt(_0x40486b(0x128), 10) / 0x7) +
        (-parseInt(_0x40486b(0x126), 10) / 0x8) * (-parseInt(_0x40486b(0x12a), 10) / 0x9) +
        (parseInt(_0x40486b(0x129), 10) / 0xa) * (parseInt(_0x40486b(0x12c), 10) / 0xb)
...
function getGUID() {
  ...
  let _0x161e32 = Date.now(),
    _0x4ec135 = _0x51cd6c(0x122)[_0x51cd6c(0x131)](/[xy]/g, (_0x32f946) => {
      const _0x216c0f = _0x51cd6c
      const _0x141041 = Math[_0x216c0f(0x125)]((_0x161e32 + Math[_0x216c0f(0x12e)]() * 0x10) % 0x10)
      _0x161e32 = Math[_0x216c0f(0x125)](_0x161e32 / 0x10)
      const _0x31fcdd = (_0x32f946 === 'x' ? _0x141041 : (_0x141041 & 0x7) | 0x8)[_0x216c0f(0x123)](
        0x10
      )
      return _0x31fcdd
    })
