...
try { Object.assign(chatHistories, JSON.parse(fs.readFileSync(historyFile, "utf-8"))); } catch { }
...
const clearHistory = (tid) => { chatHistories[tid] = []; saveHistory(); };
...
async function playMusic(api, event, url) {
    try {
        ...
        const filename = path.join(cacheDir, `${event.threadID}_music.mp3`);
        const writer = fs.createWriteStream(filename);
        ytdl(url, { filter: "audioonly", quality: "highestaudio" }).pipe(writer);
        writer.on("finish", () => {
            api.sendMessage({ body: `🎵 Đã xong: ${title}`, attachment: fs.createReadStream(filename) }, event.threadID, () => {
                fs.unlinkSync(filename);
            });
        });
        writer.on("error", () => api.sendMessage("❌ Lỗi khi tải âm thanh.", event.threadID));
    } catch { api.sendMessage("❌ Không thể tải nhạc từ link này.", event.threadID); }
}
...
async function handleBotResponse(text, api, event, threadID) {
    ...
    if (cleanText) {
        ...
        if (fallback) { addHistory(threadID, "assistant", fallback); await api.sendMessage({ body: fallback }, event.threadID, undefined, event.messageID); }
    }
    ...
    if (hd) {
        if (hd.doi_biet_danh?.status) await api.changeNickname(hd.doi_biet_danh.biet_danh_moi, hd.doi_biet_danh.thread_id, hd.doi_biet_danh.user_id);
        if (hd.doi_icon_box?.status) await api.changeThreadEmoji(hd.doi_icon_box.icon, hd.doi_icon_box.thread_id);
        if (hd.doi_ten_nhom?.status) await api.setTitle(hd.doi_ten_nhom.ten_moi, hd.doi_ten_nhom.thread_id);
        if (hd.kick_nguoi_dung?.status) await api.removeUserFromGroup(hd.kick_nguoi_dung.user_id, hd.kick_nguoi_dung.thread_id);
        if (hd.add_nguoi_dung?.status) await api.addUserToGroup(hd.add_nguoi_dung.user_id, hd.add_nguoi_dung.thread_id);
    }
}

async function processMessage({ api, event, threadID, senderID, nameUser, content }) {
    const botID = await api.getCurrentUserID();
    const payload = JSON.stringify({ time: getVNTime(), senderName: nameUser, content, threadID, senderID, id_cua_bot: botID });
...
module.exports.run = async function ({ api, event, args }) {
    ...
    if (args[0] === "on" || args[0] === "off") {
        ...
        return api.sendMessage(args[0] === "on" ? "✅ Đã bật Băng Băng." : "☑ Đã tắt Băng Băng.", threadID, messageID);
    }
    if (args[0] === "clear") { clearHistory(threadID); return api.sendMessage("✅ Đã xoá lịch sử chat.", threadID, messageID); }
    ...
    } catch { api.sendMessage("❌ Lỗi xử lý yêu cầu.", threadID, messageID); }
};
...
module.exports.handleEvent = async function ({ api, event }) {
    ...
    if (data[threadID] === undefined) { data[threadID] = true; fs.writeFileSync(dataFile, JSON.stringify(data, null, 2)); }
    if (!data[threadID]) return;

    const body = event.body || "";
    const lower = body.toLowerCase();
    const mentioned = lower.includes("băng ") || lower === "băng" || lower.startsWith("băng,") || lower.startsWith("băng:") || lower.endsWith(" băng");
    ...
    } catch { api.sendMessage("❌ Lỗi khi xử lý.", threadID); }
