...
module.exports.run = async ({ api, event, args, Currencies, utils }) => {
  ...
  if (Number.isNaN(moneys) || moneys.indexOf('-') !== -1)
    return api.sendMessage(
      'Số tiền đặt cược của bạn không phải là một con số, vui lòng xem lại cách sử dụng tại help tungdongxu',
      threadID,
      messageID
    )
  ...
  if (moneys < 10000)
    return api.sendMessage(
      'Số tiền đặt cược của bạn quá nhỏ, tối thiểu là 10000 RP',
      threadID,
      messageID
    )
  ...
  if (xu === 0 && check(random) === 'ngửa')
    api.sendMessage(
      `Bạn đã chọn ngửa, bạn đã thắng với số tiền: ${moneys * 2} RP`,
      threadID,
      () => Currencies.increaseMoney(senderID, parseInt(moneys * 2, 10)),
      messageID
    )
  else if (xu === 1 && check(random) === 'xấp')
    api.sendMessage(
      `Bạn đã chọn xấp, bạn đã thắng với số tiền: ${moneys * 2} RP`,
      threadID,
      () => Currencies.increaseMoney(senderID, parseInt(moneys * 2, 10)),
      messageID
    )
    ...
    api.sendMessage(
      `Đồng xu ra mặt ${check(random)}\nBạn đã thua và bị trừ: ${moneys} RP`,
      threadID,
