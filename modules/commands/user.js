...
module.exports.languages = {
  vi: {
    ...
    returnBan:
      '[ 𝗠𝗢𝗗𝗘 ] → Hiện tại bạn đang yêu cầu cấm người dùng:\n- ID và tên người dùng cần cấm: %1%2\n\n❮ Reaction tin nhắn này để xác thực ❯',
    returnUnban:
      '[ 𝗠𝗢𝗗𝗘 ] → Hiện tại bạn đang yêu cầu gỡ cấm người dùng:\n- ID và tên người dùng cần gỡ cấm: %1\n\n❮ Reaction tin nhắn này để xác thực ❯',
    returnBanCommand:
      '[ 𝗠𝗢𝗗𝗘 ] → Hiện tại bạn đang yêu cầu cấm sử dụng lệnh đối với người dùng:\n - ID và tên người dùng cần cấm: %1\n- Các lệnh cần cấm: %2\n\n❮ Reaction tin nhắn này để xác thực ❯',
    returnUnbanCommand:
      '[ 𝗠𝗢𝗗𝗘 ] → Hiện tại bạn đang yêu cầu gỡ cấm sử dụng lệnh đối với với người dùng:\n - ID và tên người dùng cần gỡ cấm lệnh: %1\n- Các lệnh cần gỡ cấm: %2\n\n❮ Reaction tin nhắn này để xác thực ❯',
    ...
    returnInfo:
      '[ 𝗠𝗢𝗗𝗘 ] → Đây là một số thông tin về người dùng bạn cần tìm:\n- ID và tên của người dùng: %1\n- Có bị ban: %2 %3 %4\n- Bị ban lệnh: %5',
  },
  en: {
    reason: 'Reason',
    at: 'At',
  ...
  switch (type) {
    case 'ban': {
      try {
        ...
        return api.sendMessage(getText('errorReponse', '[ 𝗠𝗢𝗗𝗘 ] → '), threadID)
      }
    }
  ...
  switch (type) {
    ...
    case '-b': {
      if (!global.data.allUserID.includes(targetID))
        return api.sendMessage(getText('IDNotFound', '[ 𝗠𝗢𝗗𝗘 ] → '), threadID, messageID)
    ...
    case '-s':
      {
        var { userName } = global.data,
          txt = '',
          count = 0
        userName.forEach((v, k) => {
          if (v.toLowerCase().includes(reason.replace(event.args[1], '').trim().toLowerCase()))
    ...
    case '-bc': {
      ...
      if (reason == null || reason.length === 0)
        return api.sendMessage(getText('missingCommandInput', '[ 𝗠𝗢𝗗𝗘 ] → '), threadID, messageID)
      if (reason === 'all') {
        var allCommandName = []
    ...
    case '-i': {
      ...
      return api.sendMessage(
        getText(
          ...
          dateAdded ? `${getText('at')}: ${dateAdded}` : '',
          commandBanned
            ? `${commandBanned.length === global.client.commands.size ? getText('allCommand') : commandBanned.join(', ')}`
            : ''
        ),
        threadID,
        messageID
