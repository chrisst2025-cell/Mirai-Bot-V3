...
➜ 𝗧𝗮́𝗰𝗵 𝘁𝗵𝗮̀𝗻𝗵 𝗰𝗼̂𝗻𝗴 𝗻𝗲̂̀𝗻 𝗰𝘂̉𝗮 𝗮̉𝗻𝗵 𝗯𝗮̣𝗻 𝘃𝘂̛̀𝗮 𝗿𝗲𝗽𝗹𝘆`
    ...
    if (event.messageReply.attachments[0].type !== 'photo')
      return api.sendMessage('➜ Đây không phải là image', event.threadID, event.messageID)

    const content =
      event.type === 'message_reply' ? event.messageReply.attachments[0].url : args.join(' ')
    const KeyApi = [
    ...
    axios({
      ...
      .then((response) => {
        ...
        return api.sendMessage(
          { body: tpk, attachment: fs.createReadStream(inputPath) },
          event.threadID,
