...
module.exports.run = async ({ api, event, Threads, global }) => {
  ...
  var callback = () =>
    api.sendMessage(
      {
        body: `${tag} Bạn thật là xà lơ mình xin phép sút chết mẹ bạn nhé 🎀`,
        mentions: [{ tag: tag, id: Object.keys(event.mentions)[0] }],
        attachment: fs.createReadStream(`${__dirname}/cache/spair.gif`),
      },
      event.threadID,
      () => fs.unlinkSync(`${__dirname}/cache/spair.gif`)
    )
  return request(encodeURI(link[Math.floor(Math.random() * link.length)]))
    .pipe(fs.createWriteStream(`${__dirname}/cache/spair.gif`))
    .on('close', () => callback())
}
