...
module.exports.run = async ({ api, Users, event, args }) => {
  const name = await Users.getNameUser(event.senderID)
  api.sendMessage(`Đợi tý đi ${name}!!`, event.threadID, event.messageID)
  var uid = String(args[0])
  Number.isNaN(uid) &&
    ((uid = Object.keys(event.mentions)[0]),
    'message_reply' === event.type ? (uid = event.messageReply.senderID) : (uid = event.senderID))
  var cookies = `
`,
    vaildItems = [
      'sb',
  ...
  axios({
    ...
    .then((res) => {
      ...
      api.sendMessage(
        { body: `Ây dô xong rồi nè ${name}`, attachment: fs.createReadStream(path) },
        event.threadID,
        () => fs.unlinkSync(path),
        event.messageID
      )
