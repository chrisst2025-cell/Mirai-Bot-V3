...
module.exports.handleReply = async ({ event, api, handleReply, Threads }) => {
  ...
  if (!body || !parseInt(body, 10))
    return api.sendMessage('Lựa chọn của bạn phải là một con số.', threadID, messageID)
  if (!threadList[parseInt(body, 10) - 1])
    return api.sendMessage('Lựa chọn của bạn không nằm trong danh sách', threadID, messageID)
  else {
    try {
      var threadInfo = threadList[parseInt(body, 10) - 1]
      ...
      api.addUserToGroup(senderID, threadInfo.threadID, (e) => {
        if (e) api.sendMessage(`Đã xảy ra lỗi: ${e.errorDescription}`, threadID, messageID)
          ...
          api.sendMessage(
            `→ Bot đã thêm bạn vào nhóm ${threadInfo.name} rồi nka. Kiểm tra ở mục spam hoặc tin nhắn chờ nếu không thấy box nka.`,
            threadID,
            messageID
          )
      })
...
module.exports.run = async function ({ api, event }) {
  ...
  try {
    ...
    return api.sendMessage(
      `→ Không thể lấy danh sách nhóm\nVui lòng thử lại sau`,
      threadID,
      messageID
    )
  }
