...
module.exports.run = async ({ api, event, args }) => {
  ...
  var text = [],
    key = global.configModule.math.WOLFRAM
  ...
  if (!content) return out('Vui lòng nhập phép tính')
      ...
      if (content.includes('from') && content.includes('to')) {
        ...
        return out(
          data.queryresult.pods
            .find((e) => e.id === 'IndefiniteIntegral')
            .subpods[0].plaintext.split(' = ')[1]
            .replace('+ constant', '')
        )
    } catch (e) {
      out(`${e}`)
    ...
    try {
      ...
      var src = data.queryresult.pods.some((e) => e.id === 'Plot')
      ...
      img
        .pipe(fs.createWriteStream('./graph.png'))
        .on('close', () =>
          api.sendMessage(
    ...
    try {
      ...
      var vector_length = data.queryresult.pods.find((e) => e.id === 'VectorLength').subpods[0]
          .plaintext,
        result
    ...
    try {
      ...
      if (data.queryresult.pods.some((e) => e.id === 'Solution')) {
        ...
        for (const e of value.subpods) text.push(e.plaintext)
