...
module.exports.onLoad = async () => {
  ...
  const pathData = join(`${__dirname}/data/game/bank.json`)
...
module.exports.run = async function ({
  ...
  try {
    ...
    const { join } = require('node:path')
    const pathData = join(`${__dirname}/data/game/bank.json`)
    const user = require('./data/game/bank.json')
    const timeIM = 60 * 60
    const laisuat = 0.05
    const _moneyInput = parseInt(args[1], 10)
    if (args[0] === '-r' || args[0] === 'register') {
      if (!user.find((i) => i.senderID === senderID)) {
        ...
        return api.sendMessage(
          `[ ✅ SUCCESS ] - Bạn đã đăng kí thành công, gửi ít nhất 10000$ để có lãi💰`,
          threadID,
          messageID
        )
      } else
        return api.sendMessage(
          `[ ⚠️ WARNING ] - Bạn đã có tài khoản trên hệ thống MIRAI Bank🏦`,
          threadID,
          messageID
        )
    }
    if (args[0] === 'check' || args[0] === 'coins') {
      if (!user.find((i) => i.senderID === senderID))
        return api.sendMessage(
          `[⚠️ WARNING] - Bạn chưa đăng kí sử dụng banking, ${global.config.PREFIX}${this.config.name} register để đăng kí🏦`,
          threadID,
          messageID
        )
      else {
        ...
        return api.sendMessage(
          `[ BANKING ] - Số tiền bạn gửi Mirai Bank là: ${formatNumber(userData.money)}$\n💷 Lãi: +${laisuat}% trong ${timeIM / 60} phút`,
          threadID,
          messageID
        )
      }
    ...
    if (args[0] === 'gửi' || args[0] === 'send') {
      if (!args[1])
        return api.sendMessage(
          '[❎ FAILED] - Số tiền gửi vào phải là một con số',
          threadID,
          messageID
        )
      if (!user.find((i) => i.senderID === senderID)) {
        ...
        if (balance < 10000)
          return api.sendMessage(
            '[ ⚠️ WARNING ] - Số tiền gửi ngân hàng phải lớn hơn 10,000',
            threadID,
            messageID
          )
        ...
        if (balance > balances)
          return api.sendMessage(
            `[ ⚠️ WARNING ] - Số dư không đủ ${formatNumber(balance)} để gửi vào Mirai Bank💰`,
            threadID,
            messageID
          )
        var userData = user.find((i) => i.senderID === senderID)
        ...
        return api.sendMessage(
          `[ ✅ SUCCESS ] - Bạn vừa gửi ${formatNumber(balance)}$ vào Mirai Bank\n💷 Lãi: +${laisuat}% trong ${timeIM / 60} phút`,
          threadID,
          messageID
        )
      }
    ...
    if (args[0] === 'rút' || args[0] === 'lấy') {
      if (!args[1])
        return api.sendMessage('[⚠️ WARNING] - Vui lòng nhập số tiền 💰', threadID, messageID)
      if (!user.find((i) => i.senderID === senderID)) {
        return api.sendMessage(
          `[⚠️ WARNING] - Bạn chưa đăng kí sử dụng banking, ${global.config.PREFIX}${this.config.name} register để đăng kí🏦`,
          threadID,
        ...
        if (BigInt(money) < 10000)
          return api.sendMessage(
            '[ ⚠️ WARNING ] - Số tiền rút ngân hàng phải lớn hơn 10,000',
            threadID,
            messageID
          )
        if (BigInt(money) > BigInt(userData.money))
          return api.sendMessage(
            '[ ⚠️ WARNING ] - Số dư của bạn không đủ để thực hiện giao dịch này!',
            threadID,
            messageID
          )
        else {
          ...
          writeFileSync(pathData, JSON.stringify(user, null, 2))
          return api.sendMessage(
            `[ BANKING ] - Rút thành công ${formatNumber(money)}$, số dư còn lại là ${userData.money}$`,
            threadID,
            messageID
          )
        }
