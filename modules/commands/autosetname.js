...
module.exports.onLoad = () => {
  ...
  const pathData = join(__dirname, 'data', 'admin', 'autosetname.json')
  const dirData = join(__dirname, 'data', 'admin')
  if (!existsSync(dirData)) require('node:fs').mkdirSync(dirData, { recursive: true })
  if (!existsSync(pathData)) return writeFileSync(pathData, '[]', 'utf-8')
}

module.exports.run = async ({ event, api, args, permssionm, Users }) => {
  ...
  switch (args[0]) {
    case 'add': {
      ...
      const _name = (await Users.getData(event.senderID)).name
