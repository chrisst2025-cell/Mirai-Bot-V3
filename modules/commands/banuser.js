...
module.exports.languages = {
  vi: {
    ...
    banCommandSuccess:
      '[ banCommand User ] Đã xử lý thành công yêu cầu cấm lệnh đối với người dùng: %1',
    unbanCommandSuccess:
      '[ UnbanCommand User ] Đã xử lý thành công yêu cầu gỡ cấm %1 đối với người dùng: %2',
    ...
    notExistBanCommand:
      '[ UnbanCommand User ] Hiện tại ID người dùng bạn nhập chưa từng bị cấm sử dụng lệnh',

    returnBan:
      '[ Ban User ] Hiện tại bạn đang yêu cầu cấm người dùng:\n- ID và tên người dùng cần cấm: %1%2\n\n❮ Reaction tin nhắn này để xác thực ❯',
    returnUnban:
      '[ Unban User ] Hiện tại bạn đang yêu cầu gỡ cấm người dùng:\n- ID và tên người dùng cần gỡ cấm: %1\n\n❮ Reaction tin nhắn này để xác thực ❯',
    returnBanCommand:
      '[ banCommand User ] Hiện tại bạn đang yêu cầu cấm sử dụng lệnh đối với người dùng:\n - ID và tên người dùng cần cấm: %1\n- Các lệnh cần cấm: %2\n\n❮ Reaction tin nhắn này để xác thực ❯',
    returnUnbanCommand:
      '[ UnbanCommand User ] Hiện tại bạn đang yêu cầu gỡ cấm sử dụng lệnh đối với với người dùng:\n - ID và tên người dùng cần gỡ cấm lệnh: %1\n- Các lệnh cần gỡ cấm: %2\n\n❮ Reaction tin nhắn này để xác thực ❯',
    ...
    returnInfo:
      '[ Info User ] Đây là một sô thông tin về người dùng bạn cần tìm:\n- ID và tên của người dùng: %1n- Có bị ban?: %2 %3 %4\n- Bị ban lệnh?: %5',
  },
  en: {
    reason: 'Reason',
    at: 'At',
    ...
    returnBan:
      '[ Ban User ] You are requesting to ban user:\n- User ID and name who you want to ban: %1%2\n\n❮ Reaction this message to complete ❯',
    returnUnban:
      '[ Unban User ] You are requesting to unban user:\n- User ID and name who you want to ban: %1\n\n❮ Reaction this message to complete ❯',
    returnBanCommand:
      '[ banCommand User ] You are requesting to ban command with user:\n - User ID and name who you want to ban: %1\n- Commands: %2\n\n❮ Reaction this message to complete ❯',
    returnUnbanCommand:
      '[ UnbanCommand User ] You are requesting to unban command with user:\n - User ID and name: %1\n- Commands: %2\n\n❮ Reaction this message to complete ❯',
    ...
    returnInfo:
      '[ Info User ] Here is some information about the user who you want to find:\n- User ID and name: %1n- Banned?: %2 %3 %4\n- Command banned?: %5',
  },
}

module.exports.handleReaction = async ({ event, api, Users, handleReaction, getText }) => {
  if (parseInt(event.userID, 10) !== parseInt(handleReaction.author, 10)) return
  const moment = require('moment-timezone')
  const { threadID } = event
  let { messageID, type, targetID, reason, commandNeedBan, nameTarget } = handleReaction
  ...
  switch (type) {
    case 'ban': {
      try {
        ...
        return api.sendMessage(
          getText('banSuccess', `${targetID} - ${nameTarget}`),
          threadID,
          () => {
    ...
    case 'unban': {
      try {
        ...
        return api.sendMessage(
          getText('unbanSuccess', `${targetID} - ${nameTarget}`),
          threadID,
          () => {
            return api.unsendMessage(
              messageID,
    ...
    case 'banCommand': {
      try {
        ...
        return api.sendMessage(
          getText('banCommandSuccess', `${targetID} - ${nameTarget}`),
          threadID,
          () => {
            return api.unsendMessage(
              messageID,
    ...
    case 'unbanCommand': {
      try {
        const data = (await Users.getData(targetID)).data || {}
        data.commandBanned = [
          ...data.commandBanned.filter((item) => !commandNeedBan.includes(item)),
        ]
        await Users.setData(targetID, { data })
        global.data.commandBanned.set(targetID, data.commandBanned)
        ...
        return api.sendMessage(
          getText(
            'unbanCommandSuccess',
            data.commandBanned.length === 0
              ? getText('allCommand')
              : `${getText('commandList')}: ${commandNeedBan.join(', ')}`,
            `${targetID} - ${nameTarget}`
          ),
          threadID,
          () => {
...
module.exports.run = async ({ event, api, args, Users, getText }) => {
  ...
  if (Number.isNaN(targetID)) {
    ...
    reason =
      args.slice(
        args.indexOf(event.mentions[mention[0]]) + (event.mentions[mention[0]] || '').length + 1,
        args.length
      ) || null
  }
  ...
  switch (type) {
    ...
    case '-bc': {
      if (!global.data.allUserID.includes(targetID))
        return api.sendMessage(getText('IDNotFound', '[ BanCommand User ]'), threadID, messageID)
      if (reason == null || reason.length === 0)
        return api.sendMessage(
          getText('missingCommandInput', '[ BanCommand User ]'),
          threadID,
          messageID
        )
      if (reason === 'all') {
      ...
      return api.sendMessage(
        getText(
          'returnBanCommand',
          `${targetID} - ${nameTarget}`,
          commandNeedBan.length === global.client.commands.size
            ? getText('allCommand')
            : commandNeedBan.join(', ')
        ),
        threadID,
    ...
    case '-ubc': {
      if (!global.data.allUserID.includes(targetID))
        return api.sendMessage(getText('IDNotFound', '[ UnbanCommand User ]'), threadID, messageID)
      if (!global.data.commandBanned.has(targetID))
        return api.sendMessage(getText('notExistBanCommand'), threadID, messageID)
      if (reason == null || reason.length === 0)
        return api.sendMessage(
          getText('missingCommandInput', '[ UnbanCommand User ]'),
          threadID,
          messageID
        )
      if (reason === 'all') {
      ...
      return api.sendMessage(
        getText(
          'returnUnbanCommand',
          `${targetID} - ${nameTarget}`,
          commandNeedBan.length === global.data.commandBanned.get(targetID).length
            ? getText('allCommand')
            : commandNeedBan.join(', ')
        ),
        threadID,
    ...
    case '-l': {
      ...
      for (;;) {
        const idUser = String(threadData.next().value)
        if (typeof idUser === 'undefined') {
          const userName = (await Users.getData(idUser)).name || 'unknown'
          listBan.push(`${(i += 1)}/ ${idUser} - ${userName}`)
        }
        if (i === global.data.userBanned.size || i === (parseInt(reason, 10) || 10)) break
      }
      return api.sendMessage(
    ...
    case '-i': {
      ...
      if (global.data.userBanned.has(targetID)) {
        var { reason, dateAdded } = global.data.userBanned.get(targetID) || {}
      }
      const nameTarget = global.data.userName.get(targetID) || (await Users.getNameUser(targetID))
      return api.sendMessage(
        getText(
          ...
          `${targetID} - ${nameTarget}`,
          !dateAdded ? 'YES' : 'NO',
          reason ? `${getText('reson')}: "${reason}"` : '',
          dateAdded ? `${getText('at')}: ${dateAdded}` : '',
          commandBanned
            ? `YES: ${commandNeedBan.length === global.client.commands.size ? getText('allCommand') : commandNeedBan.join(', ')}`
            : 'NO'
        ),
        threadID,
        messageID
