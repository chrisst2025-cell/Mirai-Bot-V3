...
module.exports.run = async ({ api, event, args, utils }) => {
  ...
  let areaKey,
    _location = {},
    dataWeather
  if (!fs.existsSync(`${__dirname}/data/assets/bgweather.jpg`)) {
    const getbg = (
      await axios.get(`https://i.imgur.com/1Rx88Te.jpg`, {
        responseType: 'arraybuffer',
      })
    ).data
    fs.writeFileSync(`${__dirname}/data/assets/bgweather.jpg`, Buffer.from(getbg, 'utf-8'))
  }
  if (!fs.existsSync(`${__dirname}/data/assets/Play-Bold.ttf`)) {
    const getfont = (
      await axios.get(
    ...
    fs.writeFileSync(`${__dirname}/data/assets/Play-Bold.ttf`, Buffer.from(getfont, 'utf-8'))
  }
  try {
    const response = (
      await axios.get(
    ...
    areaKey = data.Key
    _location = { latitude: data.GeoPosition.Latitude, longitude: data.GeoPosition.Longitude }
  } catch (_err) {
  ...
  Canvas.registerFont(`${__dirname}/data/assets/Play-Bold.ttf`, { family: 'Play-Bold' })
  const bg = await Canvas.loadImage(`${__dirname}/data/assets/bgweather.jpg`)
  ...
  const pathSaveImg = `${__dirname}/data/assets/weather.jpg`
