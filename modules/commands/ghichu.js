...
module.exports.run = async function ({ api, event, args }) {
  ...
  var _gio = moment.tz('Asia/Ho_Chi_Minh').format('HH:mm:ss || D/MM/YYYY')
  ...
  const idad = global.config.ADMINBOT[0]
  const permissions = global.config.ADMINBOT
  var _name = global.data.userName.get(event.senderID)
...
\n𝐁𝐨𝐭 đ𝐚̃ 𝐨𝐧𝐥𝐢𝐧𝐞 đ𝐮̛𝐨̛̣𝐜 𝐭𝐨̂̉𝐧𝐠 𝐜𝐨̣̂𝐧𝐠 ${hieu} 𝐆𝐢𝐨̛̀ ${simp} 𝐏𝐡𝐮́𝐭 ${rin} 𝐆𝐢𝐚̂𝐲\n[ ${moment().tz('Asia/Ho_Chi_Minh').format('HH:mm:ss || DD/MM/YYYY')} ]\n━━━━━━━━━━━━━━━━━━\n`,
        attachment: (
          await require('axios')({
            url: (
              await require('axios')('https://api-images.duytrollgame.repl.co/rin.php')
            ).data.data,
            method: 'GET',
            responseType: 'stream',
          })
  ...
  if (!text && name) {
    var _data = fs.readFile(`${__dirname}/${args[0]}.js`, 'utf-8', async (err, data) => {
