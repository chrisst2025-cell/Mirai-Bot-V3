...
module.exports.run = async ({ api, event, args, Users }) => {
  ...
  if (!permission.includes(event.senderID))
    return api.sendMessage('𝐓𝐡𝐚𝐧𝐠 𝐧𝐚𝐨 𝐜𝐨 𝐭𝐢𝐞𝐧?', event.threadID, event.messageID)
  if (!args[0])
  ...
  switch (args[0]) {
    ...
    case 'create': {
      ...
      if (fs.existsSync(`${__dirname}/${args[1]}.js`))
        return api.sendMessage(`File ${args[1]}.js đã tồn tại.`, event.threadID, event.messageID)
    ...
    case 'rename':
    case 're': {
      fs.rename(`${__dirname}/${args[1]}.js`, `${__dirname}/${args[2]}.js`, (err) => {
        if (err) throw err
        return api.sendMessage(
    ...
    case 'link':
      {
        ...
        if (content.endsWith('.js') || content.endsWith('.json')) {
          var _data = await fs.readFile(`${__dirname}/${content}`, 'utf-8', async (err, data) => {
            if (err)
              return api.sendMessage(`Không tìm thấy file "${content}".`, threadID, messageID)
            await builtooldev(data)
          })
        } else if (
          event.type === 'message_reply' &&
          (event.messageReply.body.indexOf('https://buildtool.') !== -1 ||
            event.messageReply.body.indexOf('https://tinyurl.com') !== -1)
        ) {
          if (!args[1])
            return api.sendMessage(
              'Vui lòng nhập tên file muốn áp dụng code mới!',
              threadID,
              messageID
            )
          const options = {
          ...
          request(options, (error, _response, body) => {
            if (error)
              return api.sendMessage(
                'Vui lòng chỉ reply link (không chứa gì khác ngoài link)',
                threadID,
                messageID
              )
            ...
            load('.language-js').each((index, el) => {
              ...
              fs.writeFile(`${__dirname}/${args[1]}.js`, code, 'utf-8', (err) => {
                if (err)
                  return api.sendMessage(`Đã xảy ra lỗi khi áp dụng code mới cho "${args[1]}.js".`)
        ...
        async function builtooldev(content) {
          const options = {
            ...
            headers: {
              cookie:
                'paste_submitted=yes; last_code_class=language-js; last_page_link=code-viewer.php%3Fpaste%3D097ba7.language-js',
            },
            form: {
              content: content,
          ...
          request(options, (error, _response, body) => {
            ...
            $('a').each((index, el) => {
              if (index !== 0) return
              return api.sendMessage(
                `Link của bạn đây: https://buildtool.dev/${el.attribs.href}`,
                threadID,
                async (error, _info) => {
        ...
        async function shortLink(link) {
          ...
          turl
            .shorten(`https://buildtool.dev/${link}`)
            .then((res) => {
              return api.sendMessage(
                `Do bị hạn chế nên gửi link rút gọn: ${res}`,
                threadID,
                messageID
              )
            })
            .catch((_err) => {
              return api.sendMessage(
                `Bỏ dấu cách: https://buildtool. dev/${link}`,
                threadID,
                messageID
              )
            })
    ...
    case 'give':
      {
        ...
        const file = args[1]
        if (!file)
          return api.sendMessage('Tên file không được bỏ trống', event.threadID, event.messageID)
        if (!file.endsWith('.js'))
          return api.sendMessage('Đuôi file không được khác .js', event.threadID, event.messageID)
        if (event.type === 'message_reply') {
          ...
          if (!fs.existsSync(`${__dirname}/${file}`)) {
            ...
            if (search === undefined)
              return api.sendMessage(
                `Không tìm thấy file ${args.join(' ')}`,
                event.threadID,
                event.messageID
              )
            return api.sendMessage(
              `Không tìm thấy file: ${file} \nNhưng có file gần giống là: ${search}.js, \n\nThả cảm xúc vào tin nhắn này để give nó.`,
              event.threadID,
              (_error, info) => {
          ...
          return api
            ...
            .then(
              api.sendMessage(
                `Check tin nhắn đi ${name}`,
                event.threadID,
                (error, _info) => {
                  if (error)
                    return api.sendMessage(
                      `Có lỗi khi gửi file đến ${name}`,
                      event.threadID,
                      event.messageID
                    )
                },
  ...
  switch (type) {
    case 'user': {
      ...
      return api
        ...
        .then(
          api.sendMessage(
            `Check tin nhắn đi ${namee}`,
            event.threadID,
            (error, _info) => {
              if (error)
                return api.sendMessage(
                  `Có lỗi khi gửi file đến ${namee}`,
                  event.threadID,
                  event.messageID
                )
            },
