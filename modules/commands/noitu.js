...
const path = `${__dirname}/data/game/noitu/noitu.txt`
let data = []
...
exports.onLoad = async () => {
  const dir = `${__dirname}/data/game/noitu`
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
  if (!fs.existsSync(path)) {
    data = (
      await require('axios').get(
        `https://raw.githubusercontent.com/J-JRT/api2/mainV2/linkword.json`
      )
    ).data
      .split(',')
      .filter(word_valid)
...
exports.handleReply = async (o) => {
  ...
  const word = (o.event.body || '').split(' ')
  const send = (msg, callback) =>
    o.api.sendMessage(
      msg,
      o.event.threadID,
      callback,
      callback === 0 ? undefined : o.event.messageID
    )

  if (!word_valid(word.join(' '))) return send(` ➜ Từ nối không hợp lệ!`)
  o.api.unsendMessage(
    _.messageID,
  ...
  if (_.type === 'player_vs_bot') {
    if (word[0].toLowerCase() !== _.word_bot.split(' ')[1].toLowerCase()) {
      ...
      const image_random = image_all[(Math.random() * image_all.length) << 0]
      send(
        {
    ...
    const word_matching = data.filter(
      ($) => $.split(' ')[0].toLowerCase() === word[1].toLowerCase()
    )
    const random_word_ = word_matching[(Math.random() * word_matching.length) << 0]
...
exports.run = async (o) => {
  ...
  const bet = +o.args[0] || 0
  const word_bot = data[(Math.random() * data.length) << 0]

  if (o.args[0] === 'bot') return send(` ➜ Hiện tại bot có: ${data.length} câu có thể nối!`)
  if (bet < 10000 || bet > (await o.Currencies.getData(o.event.senderID)).money)
    return send(` ➜ Bạn phải cược tiền để chơi\n[💵] ➜ Cần 10000 VNĐ để chơi!`)
