module.exports.config = {
  ...
  usages:
    '[id/name/setnamebox/emoji/me setqtv/setqtv/image/info/new/taobinhchon/setname/setnameall/rdcolor]',
  cooldowns: 0,
  dependencies: {
    axios: '',
    'fs-extra': '',
...
const totalPath = path.join(__dirname, 'data', 'config', 'totalChat.json')
...
module.exports.handleEvent = async ({ api, event, args }) => {
  ...
  if (Date.now() - totalChat[event.threadID].time > _24hours * 2) {
    const sl = (await api.getThreadInfo(event.threadID)).messageCount
...
module.exports.handleReply = ({ api, event, handleReply }) => {
  ...
  return api.createPoll(body, event.threadID, handleReply.obj, (err, _info) => {
...
module.exports.run = async function ({ api, event, args, Users, Threads }) {
  ...
  if (args[0] === 'me') {
    if (args[1] === 'qtv') {
      ...
      if (!find)
        ...
        api.sendMessage('➜ Quyền lồn biên giới ?', event.threadID, event.messageID)
  ...
  if (args[0] === 'qtv') {
    ...
    if (!finddd)
      return api.sendMessage(' ➜ Bạn méo phải quản trị viên box ?', event.threadID, event.messageID)
    if (!find) {
  ...
  if (args[0] === 'image') {
    ...
    var callback = () =>
      api.changeGroupImage(fs.createReadStream(`${__dirname}/cache/1.png`), event.threadID, () =>
  ...
  if (args[0] === 'taobinhchon') {
    ...
    const obj = {}
    for (const item of options) obj[item] = false
    api.sendMessage(
      ` ➜ Tạo thành công các bình chọn ${options.join(',')}\n ➜ Hãy phản hồi tin nhắn này để tạo tiêu đề`,
      event.threadID,
      (err, info) => {
        if (err) return console.log(err)
        else {
  ...
  if (args[0] === 'info') {
    try {
      if (!fs.existsSync(totalPath)) fs.writeFileSync(totalPath, JSON.stringify({}))
      const totalChat = JSON.parse(fs.readFileSync(totalPath))
      const threadInfo = await api.getThreadInfo(args[1] || threadID)
      const timeByMS = Date.now()
      const _threadSetting = (await Threads.getData(String(event.threadID))).data || {}
      var _memLength = threadInfo.participantIDs.length
      ...
      const preCount = totalChat[args[1] || threadID].count || 0
      const ytd = totalChat[args[1] || threadID].ytd || 0
      const hnay = ytd !== 0 ? sl - preCount : 'chưa có thống kê'
      const hqua = ytd !== 0 ? ytd : 'chưa có thống kê'
      if (timeByMS - totalChat[args[1] || threadID].time > _24hours) {
        ...
        getHour = Math.ceil(
          (timeByMS - totalChat[args[1] || event.threadID].time - _24hours) / 3600000
        )
        if (ytd === 0) mdtt = 100
