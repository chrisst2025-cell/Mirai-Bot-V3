//Ai rảnh fix lỗi download video nhé :v
  ...
  try {
    ...
    item1.continuationItems.forEach((conitem) => {
      if (conitem.itemSectionRenderer) {
        conitem.itemSectionRenderer.contents.forEach((item, _index) => {
          const videoRender = item.videoRenderer
          const playListRender = item.playlistRenderer
          if (videoRender?.videoId) {
            items.push(VideoRender(item))
...
const GetPlaylistData = async (playlistId, limit = 0) => {
  ...
  try {
    ...
    if (sectionListRenderer?.contents) {
      const videoItems =
        await sectionListRenderer.contents.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer
...
const GetSuggestData = async (limit = 0) => {
  ...
  try {
    const page = await GetYoutubeInitData(endpoint)
    const sectionListRenderer =
      await page.initdata.contents.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content
...
const GetVideoDetails = async (videoId) => {
  ...
  try {
    ...
    const res = await {
      ...
      isLive: Object.hasOwn(firstContent.viewCount.videoViewCountRenderer, 'isLive')
      ...
      suggestion: result.secondaryResults.secondaryResults.results
        .filter((y) => Object.hasOwn(y, 'compactVideoRenderer'))
...
const VideoRender = (json) => {
  if (json && (json.videoRenderer || json.playlistVideoRenderer)) {
    ...
    const channelTitle = videoRenderer.ownerText?.runs ? videoRenderer.ownerText.runs[0].text : ''
    return {
      id,
      type: 'video',
      thumbnail,
    ...
    if (dataJson.streamingData.adaptiveFormats[0].signatureCipher) {
      axios.get(`https://www.youtube.com${jsLink}`).then((i2) => {
        ...
        var format = dataDownload.map((i) => {
          ...
          i.hasVideo = !!i.qualityLabel
          i.audio = !!i.audioQuality
