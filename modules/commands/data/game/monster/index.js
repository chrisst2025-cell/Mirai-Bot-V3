...
async function createCharecter({ Users, api, event }) {
  ...
  return api.sendMessage(
    {
      body: '✅ Tạo nhân vật thành công\n\n✏️ Sử dụng lệnh /monster info để xem thông tin nhân vật\n✏️ Sử dụng lệnh /monster help để xem cách chơi',
      attachment: stream,
    },
    threadID,
    messageID
...
async function getCharacter({ api, event }) {
  ...
  if (dataUser.karma >= 40)
    statusKarma = 'Mày còn không bú ngay một chai nước thánh là mày ăn cứt nhé em, level quái +40'
  if (dataUser.karma >= 50)
    statusKarma = 'Á đù nguyên một quân đoàn ác quỷ sau lưng, level quái +50'
  if (dataUser.karma >= 60)
    statusKarma = 'Mày có chắc là không bú nước thánh không đấy, level quái +60'
  ...
  if (dataUser.karma >= 90)
    statusKarma = 'Nghe lời tao, bú nhanh một chai nước thánh đi, level quái +90'
...
async function getRank({ api, event }) {
  ...
  if (!dataUser) return api.sendMessage('❎ Bạn chưa có nhân vật', threadID, messageID)
  if (data.length === 0)
    return api.sendMessage('⚠️ Cần hạ ít nhất 1 quái để mở tính năng', threadID, messageID)
...
async function getItems({ api, event, type }) {
  ...
  switch (type) {
    case '1': {
      var msg =
        'Vũ khí loại Great Sword với lượng sát thương khủng bố 200% nhưng tốc độ giảm 50%:\n\n'
      num = 0
      ...
      var stream = (await axios.get(global.configMonster.GreatSword, { responseType: 'stream' }))
        .data
      return api.sendMessage(
    ...
    case '2': {
      var msg =
        'Các vũ khí thuộc loại Lance nổi bật với lượng DEF khủng bố, HP cao và nội tại tăng 200% DEF cho người trang bị nhưng sẽ giảm 50% tốc độ:\n\n'
      num = 0
      lance.forEach((lance) => {
        num++
    ...
    case '3': {
      var msg =
        "Các vũ khí thuộc loại Sword'n Shield  nổi bật với sự cân bằng công thủ tốc toàn diện:\n\n"
      num = 0
    ...
    case '4': {
      var msg =
        'Các vũ khí thuộc loại Dual Blades vói tốc độ khủng bố, nội tại tăng 250% tốc độ nhưng giảm thủ xuống 50%:\n\n'
      num = 0
    ...
    case '5': {
      var msg =
        'Các vũ khí thuộc loại HBG tức Heavy Bowgun vói lượng sát thương khủng cùng khả năng xuyên giáp cao, nội tại tăng 350% sát thương cùng với đó từ 30-60% xuyên giáp tuỳ cấp vũ khí nhưng giảm thủ và speed xuống 50%:\n\n'
      num = 0
      HBGs.forEach((HBGs) => {
        num++
    ...
    case '6': {
      var msg =
        'Các vũ khí thuộc loại LBG tức Light Bowgun có tốc độ cao và sát thương ổn định, đòn đánh có xuyên giáp 15% nhưng giáp bị giảm 40%:\n\n'
      num = 0
      LBGs.forEach((LBGs) => {
        num++
    ...
    case '7': {
      ...
      msg +=
        '⭐ Bạn có thể mua thức ăn bằng cách nhập số thứ tự thức ăn (có thể nhập nhiều số cách nhau bởi dấu phẩy hoặc tất cả -all)'
    ...
    case '8': {
      if (!dataUser.monster || dataUser.monster.length === 0)
        return api.sendMessage('❎ Túi của bạn không có gì', threadID, messageID)
      ...
      msg +=
        '⭐ Bạn có thể bán quái vật của mình bằng cách nhập số thứ tự quái vật (có thể nhập nhiều số cách nhau bởi dấu phẩy hoặc tất cả -all)'
    ...
    case '9': {
      ...
      msg +=
        '⭐ Bạn có thể mua bằng cách nhập số thứ tự vật phẩm nâng cấp, vô bag để sử dụng nâng cấp vũ khí đang trang bị'
...
async function buyItem({ api, event, idItem, Currencies, handleReply }) {
  ...
  try {
    switch (handleReply.id) {
      case '1': {
        if (money < handleReply.data[idItem - 1].price)
          return api.sendMessage(
            '❎ Bạn không đủ tiền, hãy chăm chỉ làm việc nhé',
            threadID,
            messageID
          )
        await Currencies.decreaseMoney(
          event.senderID,
          parseInt(handleReply.data[idItem - 1].price, 10)
        )
      ...
      case '7': {
        if (handleReply.data[idItem - 1] === undefined)
          return api.sendMessage('⚠️ Không tìm thấy vật phẩm', threadID, messageID)
        if (money < handleReply.data[idItem - 1].price)
          return api.sendMessage(
            '❎ Bạn không đủ tiền, hãy làm việc chăm chỉ nhé',
            threadID,
            messageID
          )
        await Currencies.decreaseMoney(
        ...
        return api.sendMessage(
          {
            body: `✅ Bạn đã mua thành công ${handleReply.data[idItem - 1].name} với giá ${handleReply.data[idItem - 1].price}$`,
            attachment: stream,
          },
          threadID,
          messageID
      ...
      case '9': {
        if (handleReply.data[idItem - 1] === undefined)
          return api.sendMessage('⚠️ Không tìm thấy vật phẩm', threadID, messageID)
        if (dataUser.weapon.usage >= 256)
          return api.sendMessage('❎ Vũ Khí đã đạt cấp tối đa', threadID, messageID)
        if (money < handleReply.data[idItem - 1].price)
          return api.sendMessage(
            '❎ Bạn không đủ tiền, hãy làm việc chăm chỉ nhé',
            threadID,
...
async function setItem({ api, event, handleReply }) {
  ...
  return api.sendMessage(
    { body: `✅ Đã ${weapon.type === 'weapon' ? 'đặt' : 'sử dụng'} vật phẩm`, attachment: stream },
    threadID,
    messageID
  )
}
...
async function myItem({ api, event }) {
  ...
  msg +=
    '⭐ Bạn có thể trang bị vũ khí hoặc dùng thức ăn bằng cách nhập số thứ tự của vật phẩm\n────────────────\n📌 Vũ khí mới sẽ thay thế vũ khí cũ và vũ khí cũ sẽ bị mất'
...
async function increaseDurability({ api, event, Currencies, handleReply }) {
  try {
    if (Number.isNaN(event.body))
      return api.sendMessage('❎ Vui lòng nhập một chữ số', event.threadID, event.messageID)
    if (Number.isNaN(event.body))
    ...
    if (money < event.body)
      return api.sendMessage('❎ Bạn không đủ tiền, hãy làm việc chăm chỉ nhé', threadID, messageID)
...
async function increaseHp({ api, event, handleReply }) {
  try {
    ...
    if (Number.isNaN(event.body))
      return api.sendMessage('❎ Vui lòng nhập 1 con số', event.threadID, event.messageID)
    if (dataUser.points < event.body)
      return api.sendMessage('❎ Bạn không đủ điểm, hãy chăm chỉ cày cuốc nhé', threadID, messageID)
...
async function match({ api, event }) {
  const { senderID, threadID, messageID } = event
  const _locate = require('./data/monster.json')
  const dataUser = get.getDataUser(senderID)
  if (!dataUser) return api.sendMessage('❎ Bạn chưa có nhân vật', threadID, messageID)
  if (dataUser.locationID == null)
    return api.sendMessage('❎ Bạn chưa đến khu vực nào', threadID, messageID)
  ...
  if (!monster || monster.length === 0)
    return api.sendMessage(
      '❎ Không tìm thấy khu vực này hoặc không có quái vật nào ở khu vực này',
      threadID,
      messageID
    )
  if (dataUser.weapon == null)
    return api.sendMessage(
      '❎ Bạn chưa lên đồ, bộ bạn định đánh bằng tay không à?',
      threadID,
      messageID
    )
  if (dataUser.weapon.durability <= 0)
    return api.sendMessage(
      '⚠️ Vũ khí của bạn đã bị hỏng, sửa đi rồi phang nhau tiếp nhé',
      threadID,
      messageID
    )
  if (dataUser.level < locationLevel)
    return api.sendMessage(
      `❎ Bạn chưa đạt đủ level, hãy cày thêm\nLevel khu vực: ${locationLevel}`,
      threadID,
      messageID
    )
  if (dataUser.the_luc < 50)
    return api.sendMessage(
      '⚠️ Thể lực của bạn không đủ để đánh quái vật, vui lòng ghé cửa hàng để mua thức ăn!',
      threadID,
      messageID
    )
  if (dataUser.monster.length > 30)
    return api.sendMessage('⚠️ Bạn đã đầy túi, hãy bán bớt đồ trong túi', threadID, messageID)
  ...
  if (monsterTier.length === 0)
    return api.sendMessage('Bạn không gặp quái vật', threadID, messageID)
  ...
  var threat = ''
  if (monsterRandom.HP + 4 * monsterRandom.ATK + 3 * monsterRandom.DEF + 5 * monsterRandom.SPD > 1)
    threat = '1💀'
  if (
    monsterRandom.HP + 4 * monsterRandom.ATK + 3 * monsterRandom.DEF + 5 * monsterRandom.SPD >
    4400
  )
    threat = '2💀'
  if (
  ...
  try {
    ...
    if (dur === 0)
      await api.sendMessage('⚠️ Vũ khí của bạn đã bị hỏng, sửa đi để còn phang nhau...', threadID)
    if (dataUser.weapon == null)
      await api.sendMessage('⚠️ Vũ khí của bạn đã bị hỏng nặng, chúng tôi hết cứu', threadID)
    if (dataUser.the_luc < 150)
      await api.sendMessage('⚠️ Thể lực gần cạn, chú ý bổ sung thể lực', threadID)
    var status = ''
    ...
    if (result.log.length > 30)
      status = 'Dã man tàn bạo vô nhân đạo, bạn và quái giao cấu banh cả map!!!\n\n'
...
async function listLocation({ api, event }) {
  ...
  return api.sendMessage(
    { body: msg, attachment: stream.data },
    threadID,
    (_err, info) => {
      global.client.handleReply.push({
        name: 'monster',
...
function pvp(o, id, expression) {
  const tid = o.event.threadID
  const send = (msg, cb) => o.api.sendMessage(msg, tid, cb, o.event.messageID)
  const data_user = get.getDataUser(id)
  ...
  const room = pvp_room.find(($) => $.players.includes(id))

  switch (expression) {
    case 'list rooms':
      send(
        `${pvp_room.length === 0 ? '⚠️ không có phòng do chưa có người chơi nào tạo' : pvp_room.map(($, i, _o, [p_1, p_2] = $.players.map(($) => get.getDataUser($))) => `${i + 1}. ${$.title}\n👤 Player 1: ${p_1.name} (${math.power.sum(p_1)} LC)\n👤 Player 2: ${!p_2 ? 'null' : `${p_2.name} (${math.power.sum(p_2)} LC)`}\n📝 Status: ${global.configMonster.status_room[$.status]}\n────────────────`).join('\n')}\n\nReply (phản hồi) join + stt để vào phòng pvp`,
        (_err, res) => (
          (res.name = 'monster'), (res.type = 'pvp.rooms'), global.client.handleReply.push(res)
        )
      )
      break
    case 'info room':
    ...
    case 'create room':
      {
        ...
        send(`✅ Đã tạo phòng pvp, phòng của bạn là số ${pvp_room.length}`, () =>
          pvp(o, id, 'info room')
        )
      }
      break
...
pvp.room = async (
  o,
  id = o.event.senderID,
  expression = (o.event.args || [])[0],
  stt = (o.event.args || [])[1]
) => {
  const tid = o.event.threadID
  const send = (msg, cb) =>
    new Promise((r) => o.api.sendMessage(msg, tid, cb || r, o.event.messageID))
  const data_user = get.getDataUser(id)
  const pvp_room = pvp_rooms[tid]
  ...
  switch (expression) {
    case 'join':
      {
        const room = pvp_room[stt - 1] || pvp_room[o.handleReply.stt - 1]
        ...
        room.players.push(id), (room.status = 2), (room.ready = false), pvp(o, id, 'info room')
      }
      break
    case 'start':
    case 'ready':
    case 'leave':
      {
        ...
        if (!room) return send('❎ Bạn chưa tạo or tham gia phòng pvp nào cả')
        if (room.status === 3)
          return send('⚠️ Trận pvp đang diễn ra không thể thực hiện các thao tác này!')
        if (expression === 'start' && id !== room.players[0])
          return send('❎ Bạn không phải chủ phòng để có thể bắt đầu trận pvp')
        //if (expression == 'ready' && id == room.players[0])return send('bạn là chủ phòng nên không cần sẵn sàng');
        if (expression === 'leave')
          return id === room.players[0]
            ? (pvp_room.splice(room.stt - 1, 1),
              send('✅ Đã rời phòng pvp, vì bạn là chủ phòng nên phòng sẽ bị huỷ'))
            : ((room.ready = false),
              room.status === 1,
              room.players.length === 1
                ? pvp_room.splice(room.stt - 1, 1)
                : room.players.splice(room.players.indexOf(id), 1),
              send('✅ Đã rời phòng pvp'))
        if (id === room.players[1]) {
          ...
          ;(room.status = 3), await send('🔄 PVP đang diễn ra...')

          const players = room.players.map(($) => get.getDataUser($))
          const result = require('./pvp.js')(players)
          const dmg = {
            player1: 0,
            player2: 0,
          }
