...
module.exports.onLoad = () => {
  ...
  const path = resolve(__dirname, 'data', 'shortcut', 'shortcutdata.json')
  const pathGif = resolve(__dirname, 'data', 'shortcut', 'shortcutGif')

  if (!global.moduleData.shortcut) global.moduleData.shortcut = new Map()
  if (!existsSync(resolve(__dirname, 'data', 'shortcut')))
    mkdirSync(resolve(__dirname, 'data', 'shortcut'), { recursive: true })
...
module.exports.handleEvent = async ({ event, api, Users }) => {
  ...
  if (data.some((item) => item.input === body)) {
    ...
    const path = resolve(__dirname, 'data', 'shortcut', 'shortcutGif', `${dataThread.id}.gif`)
...
module.exports.handleReply = async function ({ event, api, handleReply }) {
  ...
  switch (handleReply.type) {
    ...
    case 'requireOutput': {
      ...
      return api.sendMessage(
        "「Shortcut」Reply tin nhắn này để nhập tệp đính kèm(url có thể download) hoặc nếu không cần bạn có thể reply tin nhắn này và nhập 's'",
        threadID,
        (_error, info) =>
          global.client.handleReply.push({
            type: 'requireGif',
    ...
    case 'requireGif': {
      ...
      if (body.match(/(http(s?):)([/|.|\w|\s|-])*\.(?:gif|GIF)/g)) {
        ...
        try {
          ...
          return api.sendMessage(
            '「Shortcut」Không thể tải file vì url không tồn tại hoặc bot đã xảy ra vấn đề về mạng!',
            threadID,
            messageID
          )
        }
...
module.exports.run = function ({ event, api, args }) {
  ...
  switch (args[0]) {
    ...
    case '-d': {
      ...
      if (Number.isNaN(args[1])) indexNeedRemove = args[1]
      else
    ...
    case '-a': {
      ...
      if (data.length === 0)
        ...
        for (const single of data) {
          const path = resolve(__dirname, 'data', 'shortcut', 'shortcutGif', `${single.id}.gif`)
          var existPath = false
