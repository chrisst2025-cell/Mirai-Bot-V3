...
module.exports.onLoad = async () => {
  ...
  const path = resolve(__dirname, 'data/canvas', 'hugv3.png')
  if (!existsSync(dirMaterial)) mkdirSync(dirMaterial, { recursive: true })
  if (!existsSync(path)) {
    try {
      await downloadFile('https://i.imgur.com/7lPqHjw.jpg', path)
    } catch (e) {
      console.log(`Error downloading ôm image: ${e.message}`)
    }
  }
}
...
module.exports.run = async ({ event, api, args }) => {
  ...
  if (!mention[0]) return api.sendMessage('Please mention 1 person.', threadID, messageID)
    ...
    return makeImage({ one, two }).then((path) =>
      api.sendMessage(
        { body: 'aye', attachment: fs.createReadStream(path) },
        threadID,
        () => fs.unlinkSync(path),
        messageID
      )
